...
export function buildGenerateSetupPrompt(input: SetupPromptInput): string {
  return [
    ...
    '',
    '约束（强制）：',
    '- import: test as setup, expect（必须 expect，失败时 setup 应直接 fail）',
    '- 密码从 process.env 读取，禁止明文写死（auth lint 会扫）',
    '- 末尾必须调用 `await page.context().storageState({ path: <path> })`',
    '- 登录成功后必须有 `await expect(page).toHaveURL(/dashboard/)` 或等价断言（auth lint 会扫）',
    '',
    '最佳实践：',
    '- 用 page.getByLabel / getByRole 替代裸 selector',
    '- 失败时让 expect 抛出，不要静默 storageState 写入空 Cookie',
    '',
